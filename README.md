...
### Фото із захищеного сайту

Для фотографії, яку бібліотекар підтвердив у приватному кабінеті, є окремий
репозиторний канал. Сервер сайту створює **один** файл із чистим base64:

```text
imports/site/CAT-0112--11111111-1111-4111-8111-111111111111--ow0.jpg.b64
```

- `ow0` забороняє заміну наявної обкладинки;
- `ow1` дозволяє свідому заміну після окремого підтвердження;
- UUID є `request_id` для перевірки результату.

Workflow `import-site-photo.yml` перевіряє шлях і розмір, строго декодує
base64, відкриває файл через Pillow, створює оптимізований JPEG не більший за
600 × 900, записує `cover-status/requests/<UUID>.json` і видаляє використаний
import-файл. Очікувана помилка також отримує status JSON; небезпечний шлях або
некоректна назва файла відхиляються без видалення довільних файлів.

GitHub credential для створення import-файла має зберігатися лише на сервері
сайту. Його не можна передавати браузеру або записувати до base64-файла.
Докладний контракт: [docs/SITE_PHOTO_INGEST_UK.md](docs/SITE_PHOTO_INGEST_UK.md).
